...
async function testReadCard() {
    ...
    try {
        ...
        // Test reading card with key
        // Default MIFARE key (6 bytes of 0xFF)
        const defaultKey = 'FFFFFFFFFFFF';

        console.log('\nAttempting to read card with default key...');

        try {
            const cardContent = await reader.readCard(defaultKey);
        ...
        // You can also use a Buffer for the key
        // const keyBuffer = Buffer.from([0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
        // const cardContent = await reader.readCard(keyBuffer);
